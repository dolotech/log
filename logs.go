...
func (bl *Log) writeMsg(logLevel int, msg string, v ...interface{}) error {
	if !bl.init {
		...
		bl.setLogger(AdapterFile, `{"filename":"./logs/log.log","maxdays":1,"maxlines":50000,"maxsize":10240000}`) // 设置日志记录方式：本地文件记录
		if runtime.GOOS == "linux" {                                                                               // LINUX系统
			bl.level = LevelWarning // 设置日志写入缓冲区的等级
			bl.asynchronous = true
			bl.msgChanLen = 10000
			bl.msgChan = make(chan *logMsg, bl.msgChanLen)
			logMsgPool = &sync.Pool{
